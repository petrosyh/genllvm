...
From GenLLVM Require Import Utils Generators GenAST.
...
#[local] Instance ParamsA2 : Params := @ParamsV IPZ IPZTheory.
...
Definition assemble_runner_def (ret_t : typ) (fn_str : string) (pred_fn_blocks : runnable_blocks) : definition typ runnable_blocks
  ...
  let prototype :=
    mk_declaration name runner_typ
      ...
      []
  in
  mk_definition prototype [] pred_fn_blocks.
...
Fixpoint gen_dvalue (t : typ) : GenALIVE2 dvalue :=
  match t with
  | @TYPE_I sz =>
      iv <- lift_GenALIVE2 (choose (0, Z.min (2^(Zpos sz) - 1) 10000));;
      ret (DVALUE_Base (DVALUE_I sz (Integers.repr iv)))
  | TYPE_FP FP_float =>
      fv <- lift_GenALIVE2 fing32;;
      ret (DVALUE_Base (DVALUE_Float fv))
  | TYPE_FP FP_double =>
      failGen "Generating dvalue Double - Not supported"
  | TYPE_Vector sz subtyp =>
      ret (DVALUE_Array true (DTYPE_Array true sz (TypToDtyp.typ_to_dtyp [] subtyp))) <*> (vectorOf_ALIVE2 (N.to_nat sz) (gen_dvalue subtyp))
  | TYPE_Array sz subtyp =>
      ret (DVALUE_Array false (DTYPE_Array false sz (TypToDtyp.typ_to_dtyp [] subtyp))) <*> (vectorOf_ALIVE2 (N.to_nat sz) (gen_dvalue subtyp))
  | _ => failGen "Invalid"
  end.
